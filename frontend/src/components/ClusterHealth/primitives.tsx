  ...
  return (
    <div className="bg-oh-bg border border-oh-border rounded px-3 py-2">
      <div className="text-xs text-oh-text-muted leading-tight">
        {copyCommand ? <CopyableLabel label={label} command={copyCommand} /> : label}
        {hint && <Hint text={hint} />}
      </div>
...
/** Renders a stat label as a button that copies a shell command on click.
 *  A small copy icon fades in on hover; on click the icon swaps to a green
 *  check for 2s and the label text shows "Copied!" so the feedback is obvious
 *  without a toast system. */
function CopyableLabel({ label, command }: { label: string; command: string }) {
  ...
  return (
    <button
      ...
      className="group/copy inline-flex items-center gap-1 cursor-pointer hover:text-oh-primary transition-colors align-baseline"
    >
      <span className="underline decoration-dotted decoration-oh-border underline-offset-2 group-hover/copy:decoration-oh-primary">
        {copied ? 'Copied!' : label}
      </span>
      {copied ? (
        ...
        <svg
          className="w-3 h-3 shrink-0 opacity-0 group-hover/copy:opacity-100 transition-opacity"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
