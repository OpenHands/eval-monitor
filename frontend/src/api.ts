 ...
 *  For example:
 *  Input: "swtbench/litellm_proxy-minimax-MiniMax-M2-7/24039895569/results.tar.gz"
 *  Expected: The original run slug should be derived from params.original_run_id or similar
 */
export function getOriginalRunSlug(metadata: RunMetadata | null, currentSlug: string): string | null {
  if (!metadata?.params) return null
