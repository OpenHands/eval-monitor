...
describe('parseSearchParams', () => {
  ...
  it('returns defaults when search string is empty', () => {
    ...
    expect(result).toEqual({ date: '2025-03-17', run: null, numDays: 3, filterBenchmark: 'all', filterStatus: 'all', filterText: '', clusterHealth: false, evalTime: false, activeWorkers: false })
  })

  it('parses date from search params', () => {
    const result = parseSearchParams('?date=2025-01-15', defaultDate)
    expect(result).toEqual({ date: '2025-01-15', run: null, numDays: 3, filterBenchmark: 'all', filterStatus: 'all', filterText: '', clusterHealth: false, evalTime: false, activeWorkers: false })
  })

  it('parses run from search params', () => {
    const result = parseSearchParams('?run=swebench/model/123', defaultDate)
    expect(result).toEqual({ date: '2025-03-17', run: 'swebench/model/123', numDays: 3, filterBenchmark: 'all', filterStatus: 'all', filterText: '', clusterHealth: false, evalTime: false, activeWorkers: false })
  })
  ...
  it('parses both date and run from search params', () => {
    ...
    expect(result).toEqual({ date: '2025-02-20', run: 'gaia/litellm_proxy-gpt4/456', numDays: 3, filterBenchmark: 'all', filterStatus: 'all', filterText: '', clusterHealth: false, evalTime: false, activeWorkers: false })
  })

  it('handles URL-encoded run slugs with slashes', () => {
    const result = parseSearchParams('?run=bench%2Fmodel%2Fjob', defaultDate)
    expect(result).toEqual({ date: '2025-03-17', run: 'bench/model/job', numDays: 3, filterBenchmark: 'all', filterStatus: 'all', filterText: '', clusterHealth: false, evalTime: false, activeWorkers: false })
  })

  it('uses default date when date param is missing', () => {
    const result = parseSearchParams('?run=test/run/1', '2025-12-31')
    expect(result).toEqual({ date: '2025-12-31', run: 'test/run/1', numDays: 3, filterBenchmark: 'all', filterStatus: 'all', filterText: '', clusterHealth: false, evalTime: false, activeWorkers: false })
  })
  ...
  it('parses date, run, and days together', () => {
    ...
    expect(result).toEqual({ date: '2025-02-20', run: 'gaia/model/1', numDays: 5, filterBenchmark: 'all', filterStatus: 'all', filterText: '', clusterHealth: false, evalTime: false, activeWorkers: false })
  })

  it('parses filters', () => {
    const result = parseSearchParams('?benchmark=swebench&status=completed&text=gpt4', defaultDate)
    expect(result).toEqual({ date: '2025-03-17', run: null, numDays: 3, filterBenchmark: 'swebench', filterStatus: 'completed', filterText: 'gpt4', clusterHealth: false, evalTime: false, activeWorkers: false })
  })

  it('parses clusterHealth=true from search params', () => {
    const result = parseSearchParams('?clusterHealth=true', defaultDate)
    expect(result).toEqual({ date: '2025-03-17', run: null, numDays: 3, filterBenchmark: 'all', filterStatus: 'all', filterText: '', clusterHealth: true, evalTime: false, activeWorkers: false })
  })
  ...
  it('parses clusterHealth with other params', () => {
    ...
    expect(result).toEqual({ date: '2025-02-20', run: null, numDays: 3, filterBenchmark: 'swebench', filterStatus: 'all', filterText: '', clusterHealth: true, evalTime: false, activeWorkers: false })
  })

  it('parses evalTime=true from search params', () => {
    const result = parseSearchParams('?evalTime=true', defaultDate)
    expect(result).toEqual({ date: '2025-03-17', run: null, numDays: 3, filterBenchmark: 'all', filterStatus: 'all', filterText: '', clusterHealth: false, evalTime: true, activeWorkers: false })
  })
})
...
describe('round-trip: buildSearchString -> parseSearchParams', () => {
  ...
  it('round-trips a run selection with non-today date', () => {
    ...
    expect(parsed).toEqual({ date, run, numDays: 3, filterBenchmark: 'all', filterStatus: 'all', filterText: '', clusterHealth: false, evalTime: false, activeWorkers: false })
  })
  ...
  it('round-trips a run selection with today date', () => {
    ...
    const parsed = parseSearchParams(qs, today)
    expect(parsed).toEqual({ date: today, run, numDays: 3, filterBenchmark: 'all', filterStatus: 'all', filterText: '', clusterHealth: false, evalTime: false, activeWorkers: false })
  })
  ...
  it('round-trips list view with non-today date', () => {
    ...
    const parsed = parseSearchParams(qs, today)
    expect(parsed).toEqual({ date, run: null, numDays: 3, filterBenchmark: 'all', filterStatus: 'all', filterText: '', clusterHealth: false, evalTime: false, activeWorkers: false })
  })

  it('round-trips list view with today date', () => {
    const qs = buildSearchString(today, null, today)
    const parsed = parseSearchParams(qs, today)
    expect(parsed).toEqual({ date: today, run: null, numDays: 3, filterBenchmark: 'all', filterStatus: 'all', filterText: '', clusterHealth: false, evalTime: false, activeWorkers: false })
  })
  ...
  it('round-trips with numDays > 3', () => {
    ...
    const parsed = parseSearchParams(qs, today)
    expect(parsed).toEqual({ date, run, numDays, filterBenchmark: 'all', filterStatus: 'all', filterText: '', clusterHealth: false, evalTime: false, activeWorkers: false })
  })
  ...
  it('round-trips with numDays = 1', () => {
    ...
    const parsed = parseSearchParams(qs, today)
    expect(parsed).toEqual({ date: today, run: null, numDays: 1, filterBenchmark: 'all', filterStatus: 'all', filterText: '', clusterHealth: false, evalTime: false, activeWorkers: false })
  })

  it('round-trips with filters', () => {
    const qs = buildSearchString(today, null, today, 3, 'swebench', 'completed', 'gpt4')
    const parsed = parseSearchParams(qs, today)
    expect(parsed).toEqual({ date: today, run: null, numDays: 3, filterBenchmark: 'swebench', filterStatus: 'completed', filterText: 'gpt4', clusterHealth: false, evalTime: false, activeWorkers: false })
  })

  it('round-trips with clusterHealth=true', () => {
    const qs = buildSearchString(today, null, today, 3, 'all', 'all', '', true)
    const parsed = parseSearchParams(qs, today)
    expect(parsed).toEqual({ date: today, run: null, numDays: 3, filterBenchmark: 'all', filterStatus: 'all', filterText: '', clusterHealth: true, evalTime: false, activeWorkers: false })
  })
  ...
  it('round-trips clusterHealth with other params', () => {
    ...
    const parsed = parseSearchParams(qs, today)
    expect(parsed).toEqual({ date, run, numDays: 5, filterBenchmark: 'swebench', filterStatus: 'completed', filterText: 'gpt4', clusterHealth: true, evalTime: false, activeWorkers: false })
  })
})
